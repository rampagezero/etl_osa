...
def load_data(*args, **kwargs):
    ...
    class ApiData:
        ...
        def data_invoice_list(self,start_time,end_time):
            ...
            while more is True:
                headers={'token':'L3fhEAKpPH4T6k3IxNQDE7Es73rADCpJXEKM'}
                response=requests.get(f'https://enabler.orbiz.id/api/v1/stores/noverse/{shop_code}',headers=headers).status_code
                if response!=200:
                    self.get_access_token()
                else:
                    pass
        ...
        def get_data_payment(self):
            ...
            while j>0:
                ...
                if response!=200:
                    ...
                    pass
                timest = int(time.time())
                sign=signature(self.partner_id,path,timest,access_token,self.shop_id,self.tmp_partner_key)
                url = f"{self.host}{path}?access_token={self.access_token}&partner_id={self.partner_id}&shop_id={self.shop_id}&sign={sign}&timestamp={timest}"
                json={
    ...
    import pytz
    from datetime import datetime
    data_status_nmv=['COMPLETED']
    data_frame_final=pd.DataFrame()
    from sqlalchemy import create_engine
    engine=create_engine('postgresql://orbiz_data:0rbiz123@192.168.33.182:5433/postgres') 
    from datetime import timedelta
    import tqdm
    str_start='2025/01/15'
    str_end='2025/03/20'
    ...
    while start<end:
        for i in data_status_nmv:
            ...
            get_start=start
            get_end=get_start+timedelta(days=14)
            ...
            data_final=pd.merge(data_order,data_payment)
            data_final.rename(columns={'date':'_date','key':'_key','status':'_status'},inplace=True)
            data_final['_date']=pd.to_datetime(data_final['_date'])
            ...
            data_final.loc[:,['_date','preorder_date','shop_id','order_id','no_resi','product_id','_status','platform_id','payment_before_discount_platform','platform_discount','_key','final_payment','updated_at','quantity','invoice_ref_num']].to_sql(name='main_table',index=False,con=engine,if_exists='append')  
