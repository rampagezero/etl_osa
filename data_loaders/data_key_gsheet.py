...
def load_from_google_sheet(*args, **kwargs):
    ...
    sheet_url = 'https://docs.google.com/spreadsheets/d/1keKW8fbYaX9CNI3oct1ON4mIz0wQEuxG0wedHUK1skw/edit?gid=0#gid=0'

    return GoogleSheets.with_config(ConfigFileLoader(config_path, config_profile)).load(
        sheet_url=sheet_url,
        header_rows=1,
        worksheet_name='Sheet1',
        # expected_headers=['Market','KAM','KAE','eCustomer','Brand','Category','SKU BI','SKU ID','Full SKU ID','IT Barcode','SKU URL','SKU NAME','Status','Key','Availabilty','Key2','Not Updated']
    )
