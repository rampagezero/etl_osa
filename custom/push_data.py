...
def transform_custom(*args, **kwargs):   
    ...
    class ApiData: 
        ...
        async def get_order_detail(self): 
            ...
            #  NEED TO BE UPDATED
            shop_name_dict={1309152862:'WiZ Lighting ID',179066421:'Tefal Indonesia Official Shop',30082703:'Philips Lighting Official Shop',280207604:'IMeal Official Shop',1102188272:'Fumakilla Indonesia Store',64601228:'Nabati Official Store',912133875:'Morin Official Shop',13026781:'SKRINEER',42345408:'FiberCreme Official Shop',334913120:'Yeos'}
            engine=create_engine('postgresql://orbiz_data:0rbiz123@192.168.33.182:5433/postgres') 
            ...
            if len(data_dump_tracking_number)!=0:
                        ...
                        with engine.connect() as conn:    
                            with conn.begin():  
                                ...
                                if len(key_gather)>1:
                                    ...
                                    query=text(f"""select order_id,_key from main_table where _key  in ('{key_gather[0]}')""") 
                                ...
                                if len(order_id_map)>0:
                                    ...
                                    print(f'data updated for {store} {datetime.now()}!')  
                        
        
                        with engine.connect() as exec: 
                                with exec.begin() :  
                                        exec.execute(text("SET lock_timeout = '3s'"))  
                                        query=text('select max(order_id) from main_table ')    
                                        ...
                                        data_final.loc[:,['store_id','store_name','updated_at','shop_id']].to_sql(name='platform_table',index=False,con=engine,if_exists='append')
                                        ...
                                        exec.execute(text('''delete from platform_table st where st.id in (select id  from(select max(updated_at) over(partition by "store_id") max_1,* from platform_table )t
                where updated_at<>max_1 )''')) 
                                        print('data uploaded!!') 
