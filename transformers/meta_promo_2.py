...
def transform(data, *args, **kwargs):
    import pandas as pd
    import json
    data=data.dropna(subset='meta_payment')
    id_l=[]
    invoice_ref_num_l=[]
    ...
    data_payment_item_final_price_l=[]
    data_payment_item_quantity_l=[]
    data_subtotal={}
    for i in range(0,len(data)):
        id=data.iloc[i,0]
        invoice_ref_num=data.iloc[i,1]
        for j in range(0,len(json.loads(data.iloc[i,2]))):
            data_payment_item_id=json.loads(data.iloc[i,2])[j]['model_id']
            if data_payment_item_id==0:
                data_payment_item_id=json.loads(data.iloc[i,2])[j]['item_id']
            data_payment_item_sku=json.loads(data.iloc[i,2])[j]['model_sku']
            if data_payment_item_sku=="":
            ...
            data_payment_item_final_price=json.loads(data.iloc[i,2])[j]['discounted_price']
            data_payment_item_quantity=json.loads(data.iloc[i,2])[j]['quantity_purchased']
            id=data.iloc[i,0]
            invoice_ref_num=data.iloc[i,1]
            data_payment_item_id_l.append(data_payment_item_id)
            data_payment_item_quantity_l.append(data_payment_item_quantity)
            data_payment_item_final_price_l.append(data_payment_item_final_price)
    ...
    data_subtotal['model_id']=data_payment_item_id_l
    data_subtotal['name']=data_payment_item_name_l
    data_subtotal['sku']=data_payment_item_sku_l
    data_subtotal['quantity']=data_payment_item_quantity_l
    data_subtotal['platform_discount']=data_payment_item_shopee_discount_l
    data_subtotal['final_price']=data_payment_item_final_price_l
    data_subtotal=pd.DataFrame.from_dict(data_subtotal)
    data_subtotal['_key']=data_subtotal['id'].astype('str')+data_subtotal['sku'].astype('str')+data_subtotal['model_id'].astype('str')
